    ...
    public class GetAll_Should : BaseTest
    {
        ...
        public async Task GetAll_ShouldReturn_PaginatedCollection_With_ReportModels(int page, int perPage)
        {
            ...
            using (var assertContext = new ForumDbContext(this.Options))
            {
                ...
                Assert.AreEqual(actual.Message, "Successfully got all comment reports!");
                Assert.IsInstanceOfType(actual.Payload, typeof(Paginate<CommentReportResponseModel>));
                CollectionAssert.AllItemsAreInstancesOfType(actual.Payload.Entities.ToList(), typeof(CommentReportResponseModel));
            }
        }
    }
}